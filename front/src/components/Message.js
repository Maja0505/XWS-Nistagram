    ...
    const uploadImage = (imageForUpload, index) => {
      ...
      axios
        ...
        .then((res) => {
          ...
          socket.onopen = () => {
          console.log("Successfully Connected");                
          socket.send('{"id":true' + ',"command": 2, "channel":"' + channelName + '", "content": "","opened":false,"type":3,"text":"' +  newComment + '","user_from":"' + username + '","user_to":"' + userFromChatUsername + '"' + ',"user_for_content_id":' + '"' + username + '"' + ',"content_id":' + '"' + imageId  + '"}')
          socket.send(
            '{"user_who_follow":' +
              '"' +
              username +
    ...
    return (
        <div>
           <Button onClick={handleClickOpenNewMessageDialog}>New message</Button>
                     ...
                     {message.type === 1 &&
                       ...
                       {message.type === 3 &&
                            ...
                            <Grid container>
                                <Grid item xs={3}>
                                <Paper style={{backgroundColor:"gray", width:"100%",textAlign:"left"}}>
                                     <p style={{textAlign:"left",color:"white"}}>{message.user_from}: one-time message:<Button disabled={message.opened} onClick={() => handleOpenOneTimeMessage(message)}>Open</Button><p>{message.text}</p></p>
                                </Paper>
                                </Grid>
                                <Grid item xs={9}></Grid>
            ...
            style={{ margin: "auto" }}
          >
            {selectedFile !== undefined && selectedFile.length === 0 ? `Choose media` : `Change media`}
            <input
              hidden
            ...
            {openOneTimeMessage &&
            <OneTimeMessage open={openOneTimeMessage} setOpen={setOpenOneTimeMessage} message={imageForSend} user={loggedUserId}></OneTimeMessage>
            }
        </div>
    )
}
